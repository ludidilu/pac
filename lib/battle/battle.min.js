...
function getBattle(){
	...
	battle.init = function (mapWidth, mapHeight, mapScale, obstacle, moveSpeed, deltaTime){
	...
	battle.update = function(){
		...
		for(var id in this.heroArr){
			...
			if(hero.dir != 0){
				...
				var result = this.getHeroPos(hero.x, hero.y, hero.dir, this.moveSpeed * this.deltaTime);
	...
	battle.getHeroPos = function(heroX, heroY, heroDir, moveDistance){
		...
		var bx = Math.floor(heroX / this.mapUnitWidth);

		var lx = heroX % this.mapUnitWidth;

		var by = Math.floor(heroY / this.mapUnitWidth);
		...
		if(heroDir == 1){
			...
			if(by == 0 || (this.obstacle.v && this.obstacle.v[id - this.mapWidth])){
			...
			if(by == this.mapHeight - 1 || (this.obstacle.v && this.obstacle.v[id])){
			...
			if(bx == this.mapWidth - 1 || (this.obstacle.h && this.obstacle.h[id])){
